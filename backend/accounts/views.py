...
@api_view(["POST"])
def request_phone_code(request):
...
def register(request):
    ...
    if not (username and password and mobile_no and session_id):
        return Response({"detail": "username, password, mobile_no, verification_session_id are required"}, status=status.HTTP_400_BAD_REQUEST)
    try:
